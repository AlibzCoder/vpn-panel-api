...
module.exports = (mongoClient)=>{
    ...
    router.get('/users', (req, res) => {
        expireCheck.getUsersJoined().then(result=>{
            res.status(200).json(result);
        }).catch(err=>{
            console.log(err);
            res.status(500).json({error:"There was an error while listing the users"});
        })
    });

    router.post('/setExpire',
        ...
        (req, res) => {
            ...
            expireCheck.setExpire(ObjectId(req.body.userId),new Date(req.body.date)).then(()=>{
                res.status(200).json({message:"updated successfully"});
            }).catch(err=>{
                console.log(err);
                res.status(400).json({error : "There was an error while updating expire setting"});
            });
